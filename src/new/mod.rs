...
#[instrument(skip_all)]
pub(crate) fn new(args: &NewArgs) -> Result<()> {
...
👉 cd {output_dir} && landscape2 build --data-file data.yml --settings-file settings.yml --guide-file guide.yml --logos-path logos --output-dir dist
"
    );
}
