  ...
  return (
    <div class={`position-relative ${props.wrapperClass}`}>
      <Show when={copyStatus()}>
        <div
          class={`dropdown-menu rounded-0 popover show end-0 ${styles.tooltip} ${props.tooltipClass}`}
          role="tooltip"
        >
          <div class={`tooltip-inner rounded-0 ${styles.tooltipContent}`}>Copied!</div>
        </div>
      </Show>
      <button
        type="button"
        class={`btn btn-sm ${props.className}`}
        classList={{ [`btn-secondary rounded-0 ${styles.btn}`]: isUndefined(props.className) }}
        style={props.style}
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
