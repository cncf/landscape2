...
use landscape2_core_academic::data::{
    Commit, Contributors, GithubData, GithubOrgData, GithubOrganizationStats, Release, RepositoryGithubData,
};
    ...
    for item in &landscape_data.items {
        if let Some(url) = &item.github_org_url {
            ...
            if let Some(gh_pool) = &gh_pool {
                let gh = gh_pool.get().await.expect("token -when available-");
                github_org_stats.insert(url.to_string(), collect_github_org_stats(gh, url).await?);
            }
        }
    }
...
pub(crate) async fn collect_github_org_stats(
    gh: Object<DynGH>,
    org_url: &str,
) -> Result<GithubOrganizationStats> {
    let owner = get_owner(org_url)?;
    let repos = gh.get_owner_repos(&owner).await?;

    return Ok(GithubOrganizationStats {
